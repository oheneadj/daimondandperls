    ...
    @if($tab === 'system')
        <div class="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
            ...
            @php
                ...
                $passed      = $checklist->where('pass', true)->count();
                $total       = $checklist->count();
                $groups      = $checklist->groupBy('group');
            @endphp
            <div class="bg-white rounded-2xl border border-base-content/5 shadow-sm overflow-hidden">
                <div class="px-6 py-5 border-b border-base-content/5 flex items-center justify-between">
                    <div>
                    ...
                    <span @class([
                        'text-[11px] font-black px-3 py-1.5 rounded-full tabular-nums',
                ...
                @foreach($groups as $groupName => $checks)
                    <div class="border-b border-base-content/5 last:border-b-0">
                        <div class="px-6 py-2.5 bg-base-200/40">
                            <p class="text-[10px] font-black uppercase tracking-widest text-base-content/40">{{ $groupName }}</p>
                        </div>
                        <div class="divide-y divide-base-content/5">
                            @foreach($checks as $check)
                                <div class="px-6 py-3.5 flex items-center justify-between gap-4">
                                    <div class="flex items-center gap-3 min-w-0">
                                        ...
                                        @elseif($check['warn'])
                                            <span class="shrink-0 w-5 h-5 rounded-full bg-warning/10 flex items-center justify-center">
                                                <svg class="w-3 h-3 text-warning" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v4m0 4h.01M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/></svg>
                                            </span>
                                        @else
