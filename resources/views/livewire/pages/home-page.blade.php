<div x-data="{
    ...
    <section class="relative bg-base-100 min-h-[85vh] flex flex-col justify-center overflow-hidden">
        {{-- Subtle dot pattern --}}
        <div class="absolute inset-0 bg-[radial-gradient(circle,_#D5251820_1px,_transparent_1px)] bg-[size:28px_28px]"
            aria-hidden="true"></div>
        {{-- Red accent blob top-right --}}
        <div class="absolute top-0 right-0 size-[500px] bg-primary/8 blur-[100px] rounded-full -translate-y-1/3 translate-x-1/4"
            aria-hidden="true"></div>
        {{-- Green accent blob bottom-left --}}
        <div class="absolute bottom-0 left-0 size-[400px] bg-[#18542A]/8 blur-[100px] rounded-full translate-y-1/3 -translate-x-1/4"
            aria-hidden="true"></div>

        <div
            class="container mx-auto px-4 lg:px-8 relative z-10 pt-20 pb-16 lg:pt-28 lg:pb-24 grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8 items-center">
            <div class="lg:col-span-7">
                <div
                    class="inline-flex items-center gap-2 bg-primary/10 text-primary text-[11px] font-bold px-4 py-2 rounded-full uppercase tracking-widest mb-6">
                    <span class="size-2 rounded-full bg-primary animate-pulse"></span>
                    Accra's Finest Catering
                </div>

                <h1
                    class="text-3xl sm:text-5xl lg:text-6xl font-semibold text-base-content leading-[1.1] mb-6 tracking-tight">
                ...
                <div class="flex items-center gap-6 sm:gap-8 md:gap-12 pt-6 border-t border-base-content/8">
                    ...
                    <div>
                        <div class="flex gap-1 mb-1">
                            <svg class="size-6 sm:size-8 text-accent" fill="currentColor" viewBox="0 0 20 20">
            ...
            {{-- Hero Image Carousel --}}
            <div class="lg:col-span-5 relative hidden lg:block pl-6" x-data="{
                    current: 0,
                    slides: [
                    ...
                    <template x-for="(slide, index) in slides" :key="index">
                        <div class="absolute rounded-3xl inset-0 transition-opacity duration-1000"
                            ...
                            <img :src="slide.src" :alt="slide.alt" class="w-full h-full object-cover"
                                ...
                                width="740" height="925">
                        </div>
                    </template>

                    {{-- Gradient + caption --}}
                    <div
                        class="absolute inset-0 bg-gradient-to-t from-black/65 via-black/10 to-transparent z-20 flex items-end p-8">
                        <div>
                            <div class="inline-flex items-center gap-1.5 bg-white/20 backdrop-blur-md text-white text-[11px] font-bold px-3 py-1.5 rounded-lg uppercase tracking-widest mb-3"
                                x-text="slides[current].label"></div>
                            <h3 class="text-2xl font-bold text-white leading-tight" x-text="slides[current].caption">
                            </h3>
                        </div>
                    </div>

                    {{-- Progress bar --}}
                    <div class="absolute bottom-0 left-0 right-0 z-30 flex gap-1 px-8 pb-3">
                        <template x-for="(slide, index) in slides" :key="index">
                            <div class="h-0.5 flex-1 rounded-full bg-white/20 overflow-hidden">
                                <div class="h-full bg-white/70 rounded-full transition-all duration-300"
                                    :class="current === index ? 'w-full' : 'w-0'"></div>
                            </div>
                        </template>
                    </div>
                </div>
                ...
                {{-- Floating badge --}}
                <div
                    class="absolute -bottom-6 -left-6 bg-base-100 p-4 rounded-2xl shadow-xl flex items-center gap-4 z-20 hover:scale-105 transition-transform cursor-pointer">
                    <div
    ...
    {{-- 2. Promo Banner --}}
    <div class="bg-primary text-white py-2.5 px-4 relative" x-data="{ show: true }" x-show="show"
        x-transition.origin.top>
        {{-- Subtle shimmer line --}}
        <div class="absolute inset-0 bg-gradient-to-r from-white/0 via-white/5 to-white/0 pointer-events-none"
            aria-hidden="true"></div>
        <div
            class="container mx-auto flex items-center justify-center gap-2 md:gap-5 text-[12px] sm:text-[13px] relative">
            <span
                class="inline-flex items-center gap-1.5 bg-white/15 text-white font-bold px-2.5 py-1 rounded-full uppercase tracking-widest text-[10px] shrink-0">
                <span class="size-1.5 rounded-full bg-accent animate-pulse shrink-0"></span>
                <span class="hidden sm:inline">Special Offer</span>
                <span class="sm:hidden">Offer</span>
            </span>
            <span class="font-medium text-white/90 truncate">Book your next event and get a complimentary juice bar on
                us.</span>
            <a href="{{ route('packages.browse') }}"
                class="shrink-0 inline-flex items-center gap-1 font-bold text-white bg-white/15 hover:bg-white/25 px-3 py-1 rounded-lg transition-colors text-[11px] uppercase tracking-widest whitespace-nowrap">
                Claim Offer
            ...
            <button @click="show = false"
                class="absolute right-0 top-1/2 -translate-y-1/2 p-1.5 hover:bg-white/10 rounded-lg transition-colors"
                title="Dismiss">
    ...
    <section id="packages" class="py-16 sm:py-24 bg-base-100">
        <div class="container mx-auto px-4 lg:px-8">
            <div
                class="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 sm:mb-12 gap-4 sm:gap-8 pb-8 sm:pb-12">
                <div class="flex flex-col gap-2 relative">
                    <span class="text-[11px] font-bold text-primary uppercase tracking-[0.2em] mb-1 block">Our
                        Offerings</span>
                    <h2 class="text-3xl sm:text-4xl lg:text-5xl font-semibold text-base-content tracking-tight">Packages
                        & Pricing</h2>
                </div>
            ...
            <div class="flex items-center gap-2 overflow-x-auto pb-4 mb-8" style="scrollbar-width: none;">
                <button wire:click="selectCategory(null)" @class([
                ...
                @foreach($categories as $category)
                    <button wire:click="selectCategory({{ $category->id }})" @class([
            ...
            <div wire:loading.class="opacity-50 transition-opacity"
                class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-8 items-stretch">
                @forelse($packages as $package)
                    ...
                    @endphp
                    <div wire:key="home-pkg-{{ $package->id }}" x-data="{ pkg: @js($package), wi: @js($wi) }"
                        @click="openDetails(pkg, {{ $inCart ? 'true' : 'false' }}, wi)">
                        <x-package-card :package="$package" :selected="$inCart" :windowStatus="$ws" />
                    </div>
                @empty
                    <div class="col-span-full text-center py-16">
                        <p class="text-base-content/40 text-[15px] font-medium">No packages available in this category yet.
                        </p>
                    </div>
                @endforelse
            </div>
            ...
            {{-- Custom Menu CTA --}}
            <div
                class="mt-16 bg-primary rounded-2xl p-6 md:p-10 flex flex-col md:flex-row items-center justify-between gap-6 text-center md:text-left">
                <div>
                    <h4 class="text-xl font-bold text-white mb-2">Need a custom menu?</h4>
                    <p class="text-[14px] text-white/60 font-medium">Don't see what you need? We can create a bespoke
                        package tailored for your exact event size and budget.</p>
                </div>
                <a href="https://wa.me/{{ $whatsappNumber }}" target="_blank"
                    class="bg-accent text-neutral text-[14px] font-bold px-8 py-3.5 rounded-xl hover:brightness-110 transition-all shadow-sm shrink-0 whitespace-nowrap">
    ...
    <section class="py-16 sm:py-24 bg-base-200">
        <div class="container mx-auto px-4 lg:px-8">
            <div class="text-center mb-10 sm:mb-14">
                ...
                <h2 class="text-3xl sm:text-4xl lg:text-5xl font-semibold text-base-content tracking-tight">Events we
                    cater for</h2>
            </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
                @foreach($events as $e)
                    <a href="{{ route('packages.browse') }}"
                        class="group bg-base-100 rounded-2xl px-4 py-6 flex flex-col items-center text-center hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">
                        <div
                            class="size-12 rounded-xl bg-primary/8 text-primary flex items-center justify-center mb-3 group-hover:bg-primary group-hover:text-white transition-all duration-200">
                            <svg xmlns="http://www.w3.org/2000/svg" class="size-5" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor">{!! $e['icon'] !!}</svg>
                        </div>
        ...
        <div class="container mx-auto px-4 lg:px-8">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 sm:mb-12 gap-4">
                <div class="flex flex-col gap-2">
                    <span class="text-[11px] font-bold text-primary uppercase tracking-[0.2em] mb-1 block">In
                        Action</span>
                    <h2 class="text-3xl sm:text-4xl lg:text-5xl font-semibold text-base-content tracking-tight">Our
                        Recent Work</h2>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4">
                ...
                @foreach($galleryImages as $img)
                    <div
                        class="rounded-2xl overflow-hidden bg-base-200 aspect-square relative group shadow-sm hover:shadow-lg transition-shadow">
                        ...
                        <div
                            ...
                            <span
                                class="text-white text-[12px] font-bold uppercase tracking-widest">{{ $img['label'] }}</span>
                        </div>
                    </div>
                @endforeach
            </div>
    ...
    <section class="py-16 sm:py-24 lg:py-32 bg-primary relative overflow-hidden">
        {{-- Blobs --}}
        <div
            class="absolute top-0 right-0 size-[500px] bg-white/5 blur-[100px] rounded-full -translate-y-1/3 translate-x-1/4 pointer-events-none">
        </div>
        <div
            class="absolute bottom-0 left-0 size-[400px] bg-black/15 blur-[80px] rounded-full translate-y-1/3 -translate-x-1/4 pointer-events-none">
        </div>
        <div
            class="absolute top-1/2 left-1/2 size-[700px] bg-white/3 blur-[150px] rounded-full -translate-x-1/2 -translate-y-1/2 pointer-events-none">
        </div>
        {{-- Crosshatch texture --}}
        ...
        <div class="container mx-auto px-4 lg:px-8 relative z-10 text-center max-w-3xl">
            ...
            <div class="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4">
                <a href="{{ route('packages.browse') }}"
                    class="w-full sm:w-auto inline-flex items-center justify-center gap-2.5 bg-white text-primary text-[15px] font-bold px-8 py-4 rounded-xl hover:bg-base-100 hover:scale-105 transition-all shadow-xl shadow-black/20 text-center">
