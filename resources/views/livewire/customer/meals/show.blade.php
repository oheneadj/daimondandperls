<div>
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div class="flex items-center gap-3 sm:gap-4">
            <a href="{{ route('dashboard.meals.index') }}" wire:navigate class="size-10 bg-base-200 rounded-xl flex items-center justify-center text-base-content/60 hover:text-base-content hover:bg-base-200/80 transition-all shrink-0">
            ...
            <div class="min-w-0">
                ...
                <p class="text-[13px] text-base-content/60 font-medium mt-0.5">Meal Order Details</p>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white border border-base-content/10 rounded-2xl p-5 sm:p-6 shadow-sm">
                <h2 class="text-[11px] font-bold text-primary uppercase tracking-widest mb-5">Order Details</h2>
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-5">
                    ...
                    @if($booking->event_date)
                        <div>
                            <div class="text-[11px] font-bold text-base-content/40 uppercase tracking-wide mb-1">Delivery Date</div>
                            <div class="text-[15px] font-semibold text-base-content">{{ $booking->event_date->format('l, F j, Y') }}</div>
                        </div>
                    @endif
                    @if($booking->event_start_time)
                        <div>
                            <div class="text-[11px] font-bold text-base-content/40 uppercase tracking-wide mb-1">Delivery Time</div>
                            <div class="text-[15px] font-semibold text-base-content">{{ \Carbon\Carbon::parse($booking->event_start_time)->format('g:i A') }}</div>
                        </div>
                    @endif
                </div>
            </div>
