<div>
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
            <h1 class="text-2xl sm:text-[28px] font-semibold text-base-content leading-tight">My Events</h1>
            <p class="text-[14px] text-base-content/50 mt-1">Track your catering event bookings.</p>
        </div>
        <a href="{{ route('event-booking') }}" wire:navigate class="btn btn-primary btn-sm rounded-xl gap-2 shrink-0">
    ...
    <div class="bg-white border border-base-content/10 rounded-xl p-3 sm:p-4 mb-6 shadow-sm">
        <div class="flex flex-col sm:flex-row gap-3">
            ...
            <select wire:model.live="eventType" class="bg-base-200/50 border border-base-content/10 text-[13px] rounded-xl px-3 py-2.5 focus:ring-2 focus:ring-primary/20 focus:border-primary/30 transition-all outline-none">
                <option value="">All Event Types</option>
                @foreach(\App\Enums\EventType::cases() as $et)
                    <option value="{{ $et->value }}">{{ str($et->value)->title()->replace('_', ' ') }}</option>
                @endforeach
            </select>
            @if($search || $status || $paymentStatus || $eventType)
    ...
    @if($bookings->isEmpty())
        <div class="bg-white border border-base-content/10 rounded-2xl p-10 sm:p-12 text-center shadow-sm">
            <div class="size-20 bg-base-200 rounded-full flex items-center justify-center mx-auto mb-6 text-primary/20">
                @include('layouts.partials.icons.clipboard-document-list', ['class' => 'size-10'])
            </div>
            <h3 class="text-xl font-semibold text-base-content mb-3">No events found</h3>
            <p class="text-base-content/60 text-[14px] max-w-md mx-auto mb-8 font-medium">
                @if($search || $status || $eventType)
                    No events match your filters. Try adjusting your search criteria.
                @else
                    You haven't made any event bookings yet. Let us cater your next event.
                @endif
            </p>
            @unless($search || $status || $eventType)
    ...
    @else
        <div class="space-y-4">
            @foreach($bookings as $booking)
                <div wire:key="event-{{ $booking->id }}" class="bg-white border border-base-content/10 rounded-2xl shadow-sm overflow-hidden hover:shadow-md transition-all">
                    ...
                    <div class="px-4 sm:px-6 py-4 flex items-center justify-between border-b border-base-content/5 bg-base-200/20">
                        ...
                        @endphp
                        <span class="px-2.5 py-1 rounded-full {{ $statusConfig['color'] }} text-[10px] font-black uppercase tracking-widest shrink-0">
                            {{ $statusConfig['label'] }}
                        </span>
                    </div>
                    ...
                    <div class="px-4 sm:px-6 py-4">
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
                            ...
                            <div class="col-span-2">
                                <div class="text-[10px] font-bold text-base-content/40 uppercase tracking-widest mb-1">Event Date</div>
                            ...
                            {{-- Time --}}
                            <div>
                                <div class="text-[10px] font-bold text-base-content/40 uppercase tracking-widest mb-1">Time</div>
                                <div class="text-[14px] font-semibold text-base-content">
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-3 border-t border-base-content/5">
                            <div class="flex items-center gap-4">
                                ...
                                @else
                                    <span class="text-[13px] font-semibold text-base-content/50 italic">Quote Pending</span>
                                @endif
                            </div>
                            <div class="flex items-center gap-2">
                                @if($booking->total_amount > 0 && $booking->payment_status === \App\Enums\PaymentStatus::Unpaid)
                                ...
                                @endif
                                <a href="{{ route('dashboard.events.show', $booking->reference) }}" wire:navigate class="inline-flex items-center gap-1.5 px-3 py-2.5 rounded-xl bg-[#18542A]/10 text-[#18542A] text-[12px] font-bold hover:bg-[#18542A]/20 transition-colors min-h-[44px]">
