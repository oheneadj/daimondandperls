    ...
    >
        ...
        >
            ...
            <template x-if="packageInCart">
                <div class="absolute top-6 left-6 z-20 animate-in zoom-in duration-300">
                    <div class="bg-success text-white size-10 rounded-full flex items-center justify-center shadow-lg border-4 border-white">
                        <svg xmlns="http://www.w3.org/2000/svg" class="size-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3">
            ...
            <div class="relative h-56 bg-base-200 flex items-center justify-center overflow-hidden shrink-0">
                ...
                <button @click="showDetails = false" class="absolute top-6 right-6 w-10 h-10 rounded-full bg-black/20 hover:bg-black/40 text-white flex items-center justify-center backdrop-blur-md transition-all z-20">
            ...
            <div class="p-8 lg:p-10 flex-1 overflow-y-auto">
                ...
                <p class="text-[15px] text-base-content/60 leading-relaxed mb-8" x-text="selectedPackage?.description"></p>
                ...
                <div class="space-y-8">
                    ...
                    <div class="grid grid-cols-2 gap-8 pt-8 border-t border-base-content/5">
                        ...
                        <div>
                            <div class="text-[11px] font-black uppercase tracking-[0.2em] text-primary mb-2">{{ __('Min Serves for events') }}</div>
            ...
            <div class="p-8 bg-base-200/50 flex gap-4 shrink-0">
                <button @click="showDetails = false" class="flex-1 py-4 border-2 border-black text-black font-black text-[13px] uppercase tracking-wider rounded-full border border-base-content/10 shadow-sm">{{ __('Close') }}</button>
                <button 
                    @click="{{ $wire }}.toggleSelection(selectedPackage.id); packageInCart = !packageInCart"
                    :class="packageInCart ? 'bg-black' : 'bg-primary'"
                    class="flex-[2] py-4 text-white font-black uppercase tracking-wider rounded-full shadow-lg shadow-primary/20 hover:scale-[1.02] transition-all active:scale-95"
                >
